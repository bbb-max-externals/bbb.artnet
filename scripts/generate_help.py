...
EXTERNALS = {
    "bbb.artnet.controller": {
        ...
        "messages": [
            ...
            ("dump", "Output current DMX buffer as list; remap metadata first when enabled"),
            ("dump_universe 1", "Output one logical Art-Net universe/port-address: universe <N> <values...>"),
        ...
        "attributes": [
            ...
            ("universe", "Art-Net universe (0-15), used when remap disabled", 260.0),
            ("input_universe_start", "First logical universe for remap", 220.0),
            ("output_universe_start", "1-based output universe; 0 disables remap", 280.0),
            ...
            ("mode", "0=auto 1=bang 2=update 3=change 4=forced", 240.0),
            ("framerate", "Target framerate (0.01-44)", 160.0),
            ("target_ip", "Destination IP (empty=broadcast)", 200.0),
            ("bind_ip", "Local bind IP (empty=auto)", 200.0),
            ("send_only", "Ephemeral ArtDmx sender; no 6454 bind/listen", 280.0),
            ("origin", "Channel index origin (0 or 1)", 180.0),
            ("osc_port", "OSC receive port (0=disabled)", 180.0),
            ("osc_bind_ip", "OSC listen address", 180.0),
            ("verbose", "Enable verbose logging", 160.0),
        ],
        "external_text": "bbb.artnet.controller @target_ip 192.168.1.100 @input_universe_start 1 @output_universe_start 101",
        "numoutlets": 1,
        "outlettype": [""],
    ...
    "bbb.artnet.node": {
        ...
        "attributes": [
            ...
            ("mode", "0=auto 1=update 2=bang 3=change 4=forced", 240.0),
            ("framerate", "Output framerate (0.01-44)", 160.0),
            ("target_ip", "Destination IP for filter", 200.0),
            ("bind_ip", "Local bind IP (empty=auto)", 200.0),
            ("osc_port", "OSC receive port (0=disabled)", 180.0),
            ("osc_bind_ip", "OSC listen address", 180.0),
            ("verbose", "Enable verbose logging", 160.0),
    ...
    "bbb.artnet.rdm": {
        ...
        "external_text": "bbb.artnet.rdm @universe 1",
        "numoutlets": 2,
        "outlettype": ["", ""],
        "outlet_labels": [
    ...
    "bbb.sacn.controller": {
        ...
        "messages": [
            ...
            ("set_offset 10 255", "Set value at offset position"),
            ("dump", "Output current DMX buffer as list; remap metadata first when enabled"),
            ("dump_universe 1", "Output one logical sACN universe: universe <N> <values...>"),
            ("set_universe 1 255 128 0", "Store one logical sACN universe without sending"),
        ],
        "attributes": [
            ("universe", "sACN universe (1-63999), used when remap disabled", 300.0),
            ("input_universe_start", "First logical universe for remap", 220.0),
            ("output_universe_start", "Output sACN universe; 0 disables remap", 280.0),
            ("num_universes", "Number of universes to span", 160.0),
            ("num_channels", "Number of DMX channels", 160.0),
            ("sync_universes", "Sync all universes on change", 160.0),
            ("blackout", "Send all zeros", 130.0),
            ("mode", "0=automatic 1=bang 2=update 3=change 4=forced", 270.0),
            ("framerate", "Target framerate", 160.0),
            ("priority", "sACN priority (0-200)", 160.0),
            ("source_name", "sACN source name", 200.0),
            ("target_ip", "Destination IP (empty=per-universe multicast)", 260.0),
            ("bind_ip", "Local interface IP / multicast interface", 260.0),
            ("send_only", "Accepted for Art-Net API parity; sACN is always send-only", 330.0),
            ("origin", "Channel/setchannel/set_offset origin", 240.0),
            ("unicast", "Legacy unicast mode", 150.0),
            ("unicast_ip", "Legacy unicast destination", 210.0),
        ],
        "external_text": "bbb.sacn.controller @target_ip 127.0.0.1 @num_universes 13 @mode 4 @output_universe_start 101",
        "numoutlets": 1,
        "outlettype": [""],
        "outlet_labels": ["bang on send; list on dump"],
    },
